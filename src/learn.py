...
train, test = preprocessing.split_dataset(dataset)
...
for i in trange(100, desc="Predicting"):
    example = preprocessing.create_next_seq(train_predict, look_back=look_back)
    example = np.reshape(example, (example.shape[0], 1, example.shape[1]))
    pred = model.predict(example)
    train_predict = np.append(train_predict, [pred[-1]], axis=0)
